...
sem_t WriterSem, ROISem, PROCSem;
...
static long writeaddr(subRecord *precord){
  ...
  switch(addr){
    ...
    case 21:
      if(precord->val>60){
        jfdata.exposure=60;
      }else if(precord->val<1){
        jfdata.exposure=1;
      }else{
        jfdata.exposure=(int)precord->val;
      }
      break;
    case 22:
      jfdata.BGEnable=(int)precord->val;
      break;
    case 23:
...
static long readaddr(subRecord *precord){
  ...
  switch(addr){
    ...
    case 20:
      ...
      break;
    case 21:
      precord->val=jfdata.validframestat;
      break;
    case 22:
    ...
    case 23:
      ...
      break;
    case 24:
      precord->val=(double)jfdata.exposure;
      break;
    case 25:
      precord->val=(double)jfdata.totalsum;
      break;
    case 26:
      precord->val=(double)jfdata.summax;
      break;
    case 27:
      precord->val=(double)jfdata.summin;
      break;
    case 28:
      precord->val=(double)jfdata.BGSave_RBV;
      break;
    case 29:
      precord->val=(double)jfdata.ProcTime;
      break;
    default:
      precord->val=0;
      break;
...
static long readarrays(aSubRecord *precord){
    ...
    if(gpool.dataready==1){
      gpool.dataready=0;
      memcpy(precord->vala, jfdata.rawadu, sizeof(double)*NPIXELS);
...
static long startserver(subRecord *precord){
  ...
  // Init thread 1 - Process server
  tid[1]=epicsThreadCreate("Process_Server", epicsThreadPriorityMedium, epicsThreadGetStackSize(epicsThreadStackMedium), procserver, 0);
  if (!tid[1]){
...
static void zmqserver(void *ctx){
  ...
  // create buffers
  jfdata.rawbuffer[0] = (char *)malloc(datablock);
  jfdata.rawbuffer[1] = (char *)malloc(datablock);
  ...
  validframes = (uint32_t *)(tmpframe+12);
    ...
    switch(jfdata.detstate){
      case 0:
        ...
        if(size==DATA_SIZE){
          ...
          if(*validframes==128){
            index=0;
            memcpy(&jfdata.rawbuffer[blockindex][DATA_SIZE*index], tmpframe, DATA_SIZE);
...
static void procserver(void *ctx){
  ...
  while(1)
  {
    ...
    if(++exposure>=jfdata.exposure){
      ...
      summax=0;
      summin=1E200;
      // sum frame processing
      for(int i=0; i<NPIXELS; i++)
      {
        val=framesum[i];
        totalsum+=val;
        if(val>summax) summax=val;
        if(val<summin && val!=0) summin=val;
      }
      //BG saving
      if(jfdata.BGSave_RBV==1){
        for(int i=0; i<NPIXELS; i++){
          jfdata.BGFrame[i]=round(framesum[i]/totalvalidframes);
        }
        jfdata.BGSave_RBV=2;
      }

      jfdata.totalsum=totalsum;
      jfdata.summax=summax;
      jfdata.summin=summin;
      totalsum=0;
      jfdata.validframestat=totalvalidframes/(exposure*10);
      gpool.dataready=1;
      jfdata.liveframeid++;
      totalvalidframes=0;
      //reset framesum
    ...
    avgtime = (double) (ts2.tv_usec - ts1.tv_usec) / 1000.0 + (double) (ts2.tv_sec - ts1.tv_sec)*1000.0;
    jfdata.ProcTime=avgtime;
    //printf ("Processing time = %.3f msecs\n", avgtime);
  }
}
